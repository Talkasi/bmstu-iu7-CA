...
double spline(table_t &table, double x)
{
	spline_t data;
	...
	init_spline_coef_c_gauss(data);
	assert(table.size() == data.c.size());

	init_spline_coef_b(data);
	assert(table.size() == data.b.size());

	init_spline_coef_d(data);
...
void init_spline_coef_b(spline_t &data)
{
	...
	h_i = data.table[i][X] - data.table[i - 1][X];
	data.b.push_back((data.table[i][Y] - data.table[i - 1][Y]) / h_i - h_i * 2 * data.c[i] / 3);
}
...
void init_spline_coef_c_gauss(spline_t &data)
{
	...
	init_rside_matrix(rside, data.table);
...
void init_spline_coef_d(spline_t &data)
{
	...
	data.d.push_back(-data.c[i] / (3 * (data.table[i][X] - data.table[i - 1][X])));
}
